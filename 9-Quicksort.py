...
def quicksort(lst, start, end):
    if start < end:
        split = pivotPartition(lst, start, end) # <pivot, pivot (position determined), >pivot
        # print (lst[start:end+1])
        quicksort(lst, start, split - 1) # quicksort <pivot
        quicksort(lst, split + 1, end) # quicksort >pivot
    else:
        return
